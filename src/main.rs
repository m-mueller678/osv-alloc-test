...
impl PagingAllocator {
    fn new() -> Self {
        ...
        let ret = PagingAllocator {
            ...
            current_page: MappedPageInfo {
                allocation_count: 0,
                frame: PhysFrame::containing_address(PhysAddr::new(0)),
            },
        };
...
fn test_alloc<const VALIDATE: bool, const VALIDATE_FULL: bool>(
    total_allocs: usize,
    avg_alloc_size: usize,
    max_concurrent_size: usize,
    a: &mut impl TestAlloc,
) {
        ...
        for i in 0..total_allocs {
            if allocs.len() == concurrent_allocs {
                let (ptr, len) = {
                    ...
                    if VALIDATE {
                        assert!(if VALIDATE_FULL { &x[..] } else { &x[..1] }
                            .iter()
                            .all(|x| x.assume_init_read() == old_id))
                    }
    ...
    println!(
        "complete. {:.3e} alloc/s",
        total_allocs as f64 / duration.as_secs_f64()
    );
}
